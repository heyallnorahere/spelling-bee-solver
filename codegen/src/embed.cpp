...
static template<typename T> bool __equals__(const T& a, const T& b) {
    return a == b;
}
static template<typename Container, typename Element> bool contains(const Container& container, const Element& element, std::function<bool(const Element&, const Element&)> equals = __equals__<Element>) {
